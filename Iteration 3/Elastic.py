...
data_file = open("Iteration 3/density_xe+2.dat", "r")
...
if lmax < 1000:
    ...
    hcon = 4.0*pi/p1/p1
    tcs=0.0
    phase_file = open("Iteration 3/Phase shifts.txt", "w")
    for i in range(lmin1, lmx + 1, lspc):
        l = i - 1
        logder = z11[i] / p1
        fhyp = f[l]
    ...
    phase_file.close()
    crosssection_file = open("Iteration 3/Cross sections.txt", "w")
    for theta in range(0, 181):
        ...
        cratio = cgamma( 1.0+ci*etahyp ) / cgamma( 1.0-ci*etahyp )
        cdexpon = cmath.exp( -ci*etahyp*math.log(sint22) )
